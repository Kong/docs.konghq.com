...
async function createPDF(name, urls) {
  ...
  for (const url of urls) {
    ...
    if (result.status() !== 404) {
      ...
      await page.evaluate(() => {
        const header = document.querySelector(".page-header--info");
        if (!header) {
          return;
        }
        const versionsDropdown = header.querySelector(".versions-dropdown");
        versionsDropdown.parentNode.removeChild(versionsDropdown);

        const content = document.querySelector(".page-content-container");
