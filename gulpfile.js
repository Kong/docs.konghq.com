...
gulp.task('pdk-docs', function (cb) {
  ...
  pdkRegex = /[ ]+- text: Plugin Development Kit Reference[\s\S]+\n-/gm
  ...
  refDir = 'app/docs/' + KONG_VERSION + '/pdk-reference'
  ...
  for (let module of modules) {
    cmd = 'LUA_PATH="$LUA_PATH;./?.lua" ' +
      ...
      module.file + ' && ' +
      'mv ./' + module.generated_name + '.md ' + refDir + '/' + module.name + '.md'
    obj = childProcess.spawnSync(cmd, { shell: true })
    errLog = obj.stderr.toString()
    if (errLog.length > 0) {
      return cb(errLog)
  ...
  gutil.log('Re-generated docs inside ' + refDir)
})
