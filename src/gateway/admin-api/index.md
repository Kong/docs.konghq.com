...
route_body: |
    ...
    `paths`<br>*semi-optional* |  A list of paths that match this Route.  With form-encoded, the notation is `paths[]=/foo&paths[]=/bar`. With JSON, use an Array. The path can be a regular expression, or a plain text pattern. The path patterns are matched against normalized path, with most percent encoded characters decoded, and path folded while preserving semantics. Check [rfc3986](https://datatracker.ietf.org/doc/html/rfc3986#section-6) for more details.
