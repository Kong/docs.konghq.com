...
| 2    | Configuration sync    | - Change decK to work with the green environment, and execute `deck gateway sync` to push the configuration to the green environment<br>- Change the Portal CLI to work against the green environment, and execute `portal deploy` to push the Dev Portal configuration against the new environment |
