...
2. Click a {{site.base_gateway}} version from the list to download it.
    ...
    For example: `kong-enterprise-edition-{{page.kong_versions[13].version}}.el8.noarch.rpm`
...
3. Copy the RPM file to your home directory on the CentOS system.
    ...
    ```bash
    $ scp kong-enterprise-edition-{{page.kong_versions[13].version}}.el8.noarch.rpm <centos user>@<server>:~
    ```
