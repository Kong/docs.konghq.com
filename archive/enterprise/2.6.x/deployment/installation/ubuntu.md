...
2. Select a {{site.base_gateway}} version from the list to download it.
    ...
    For example: `kong-enterprise-edition_{{page.kong_versions[13].version}}_all.deb`

6. Copy the `.deb` file to your home directory on the Ubuntu system. For example:

    ```bash
    $ scp kong-enterprise-edition_{{page.kong_versions[13].version}}_all.deb <ubuntu_user>@<server>:~
    ```
