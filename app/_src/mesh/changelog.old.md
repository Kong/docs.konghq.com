...
---
<!-- TODO remove this file once https://github.com/Kong/docs.konghq.com/pull/5797 is merged as we can use the one directly from the Kong-Mesh repo -->
<!-- Autogenerated with (github.com/kumahq/ci-tools) release-tool changelog.md -->

## 2.3.0
> Released on 2023/06/23

* chore(deps): bump github.com/Kong/kauth-api from 1.94.0 to 1.100.0
* chore(deps): bump github.com/aws/aws-sdk-go from 1.44.241 to 1.44.268
* chore(deps): bump github.com/cert-manager/cert-manager from 1.11.0 to 1.12.0
* chore(deps): bump github.com/docker/docker from 23.0.3+incompatible to 24.0.0+incompatible
* chore(deps): bump github.com/hashicorp/vault/api from 1.9.0 to 1.9.1
* chore(deps): bump github.com/hashicorp/vault/sdk from 0.8.1 to 0.9.0
* chore(deps): bump github.com/testcontainers/testcontainers-go from 0.18.0 to 0.19.0
* chore(deps): bump kumahq/kuma from d98ca8aacc47 to c96910d2e (Kuma 2.3.0)
* chore(deps): bump sigs.k8s.io/controller-runtime from 0.15.0-beta.0 to 0.15.0 
* chore(deps): bump tibdex/github-app-token from 1.7.0 to 1.8.0
* chore(deps): bump ubi9-minimal from 9.1.0-1829 to 9.2-484
* chore(deps): use latest Kong/kong-mesh-gui
* feat(MeshGlobalRateLimit): add header based rate limiting
* feat(MeshGlobalRateLimit): add ratelimit service auth
* feat(MeshGlobalRateLimit): allow to configure MeshSubset in top level targetRef
* feat(MeshGlobalRateLimit): secure communication between ratelimit service and DPP with TLS
* feat(MeshGlobalRateLimit): securing communication between DPP and ratelimit service MADR
* feat(MeshOPA): support builtin gateway listeners
* feat(kuma-cp): added authz integration with kauth-pdp for api-server
* feat(kuma-cp): alternative store that supports multitenancy
* feat(kuma-cp): rename variable and add helm config
* fix(MeshOPA): apply policy to correct inbounds
* fix(helm): add cert-manager RBAC
* fix(helm): update HPA API version
* fix(kauth): refresh service client tokens
* fix(konnect): remove Bearer from the token
* fix(kuma-cp): change the order of columns in primary key
* fix(kuma-cp): fixed naming of a path
* fix(kuma-cp): fixed naming of envs and added missing env def
* fix(kuma-cp/run): don't fail if valid kuma-cp args are passed

## 2.2.2
> Released on 2023/06/21

* chore(deps): bump kumahq/kuma from e30ace1c5856 to 2e775e96a (Kuma 2.2.2)
* chore(deps): use latest Kong/kong-mesh-gui

## 2.2.1
> Released on 2023/05/10

* chore(deps): bump kumahq/kuma from 9a2812c6b3a4 to e30ace1c5
* chore(deps): use latest Kong/kong-mesh-gui
