...
You can configure external authorization filter by adjusting `authConfig` section.
...
In the organisation, mesh operator may want to set policy for subset of proxies in the mesh.
At the same time, service owners may want to exercise additional policies.

For example, mesh operator may want to enable JWT token validation for all proxies in the mesh
```yaml
apiVersion: kuma.io/v1alpha1
kind: MeshOPA
metadata:
...
```
and configure OPA agent decision path (`plugins.envoy_ext_authz_grpc.path`) to `implicitkmesh/allow`.
...
This way, mesh operator can expose utility functions to service owner.
{% endif_version %}
