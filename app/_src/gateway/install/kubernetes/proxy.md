...
To make the admin API accessible without using `kubectl port-forward`, you can create an internal load balancer on your chosen cloud. There are guides available for [Amazon EKS](/gateway/{{ page.release }}/install/kubernetes/cloud/eks/), [Google GKE](/gateway/{{ page.release }}/install/kubernetes/cloud/gke/) and [Azure AKS](/gateway/{{ page.release }}/install/kubernetes/cloud/aks/). This is required to [use Kong Manager]({{ page.book.next.url }}) to view or edit your configuration.