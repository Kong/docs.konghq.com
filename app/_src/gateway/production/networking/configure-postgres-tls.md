...
```
For more information about these parameters, see the {{site.base_gateway}} [PostgreSQL settings documentation](/gateway/latest/reference/configuration/#postgres-settings), as well as the specific documentation for [`lua_ssl_trusted_certificate`](/gateway/latest/reference/configuration/#lua_ssl_trusted_certificate) and [`lua_ssl_verify_depth`](/gateway/latest/configuration/#lua_ssl_verify_depth). 
