...
{:.note}
> **Note:** You must enable SSL connection on your Postgres DB instance. After enabling the IAM database authentication, you cannot disable the SSL connection.
...
For more details, please check the AWS documentation: https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.IAMPolicy.html
...
The AWS IAM Authentication also has some limitations, some come from the AWS service and some are within the Kong runtime. Be sure to go through them one by one before you start to use this feature in your production environment.

- According to the AWS documentation, the AWS IAM Authentication to the RDS will work "when your application requires fewer than 200 new IAM database authentication connections per second". Establishing database connections more than that number in every second may result in connection throttling, controlled by the IAM service. Note that this does not affect the total number of keep-alive connections from Kong to the database, it only affects the connection establishment. If you're using a traditional cluster deployment with many Kong nodes, make sure that your number of connection establish action in your whole cluster is lower than the maximum value of 200.
