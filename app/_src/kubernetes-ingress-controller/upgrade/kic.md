...
Learn about the changes required when upgrading an {{ site.kic_product_name }} instance from 2.x to 3.x.
...
{:.note}
> **Note:** Deploying and upgrading using the [Helm chart][chart] is the
supported mechanism for production deployments of {{ site.kic_product_name }}. If you're deploying {{ site.kic_product_name }}
...
{{ site.kic_product_name }} 2.7 includes {{site.base_gateway}} 3.x compatibility changes to the controller CRDs.
Helm does not update CRDs automatically.
You must apply them manually before upgrading {{ site.kic_product_name }}.

```bash
kubectl apply -f https://raw.githubusercontent.com/Kong/charts/main/charts/kong/crds/custom-resource-definitions.yaml
```

## Upgrade your {{ site.kic_product_name }} version

{{ site.kic_product_name }} 2.7 is the first version that supports Kong 3.x. You must upgrade to {{ site.kic_product_name }}
...
Because {{ site.kic_product_name }} 2.7 is compatible with all 2.x {{site.base_gateway}} releases, you should upgrade it and
the chart first.
...
but most don't affect its interactions with {{ site.kic_product_name }}, or are handled automatically
by 2.7 changes. You should still review the changelog for changes that don't
interact with {{ site.kic_product_name }}, such as changes to `kong.conf` or environment variable settings,
...
expression paths, those paths break if you upgrade to 3.x without updating configuration.
...
process and allow users to update rules gradually, {{ site.kic_product_name }} 2.7 includes the
`enableLegacyRegexDetection` option to continue applying the 2.x regular
expression heuristic on KIC's end.

If the following sets of rules are met, {{ site.kic_product_name }} creates a Kong route path with
the `~` prefix:
