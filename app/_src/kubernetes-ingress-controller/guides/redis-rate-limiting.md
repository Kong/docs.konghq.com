...
Kong can rate limit traffic without any external dependency. Kong stores the request counters in-memory and each Kong node applies the rate limiting policy independently without  synchronization of information. However, if Redis is available in your cluster, Kong can
...
1.  Ceate a password Secret and replace `<PASSWORD>` with a password of your choice.
