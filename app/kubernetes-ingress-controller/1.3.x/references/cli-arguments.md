...
| --apiserver-host                     |`string`   | none                            | The address of the Kubernetes API server to connect to in the format of protocol://address:port, e.g., "http://localhost:8080. If not specified, the assumption is that the binary runs inside a Kubernetes cluster and local discovery is attempted.|
...
| --dump-config                        |`string`   | none                            | Dump generated configuration to a temporary directory when set to `enabled`.  When set to `sensitive`, dumps include certificate+key pairs and credentials.|
...
| --publish-service                    |`string`   | none                            | The namespaces and name of the Kubernetes Service fronting the {{site.kic_product_name}} in the form of namespace/name. The controller will set the status of the Ingress resources to match the endpoints of this service. In reference deployments, this is kong/kong-proxy.|
