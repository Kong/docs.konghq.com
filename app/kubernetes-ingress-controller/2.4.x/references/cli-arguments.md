...
| --skip-ca-certificates                 | `boolean`          | Disable CA certificate handling. When using multiple controllers for separate workspace in the same Kong instance, all but one controller should have this set to `true` | `false`      |
| --term-delay                           | `duration`         | Delay between receiving SIGTERM or SIGINT and shutdown, to allow gracefully draining instances from a LoadBalancer                                  | `0s`                              |
