...
itself without proxying the request to a upstream service.

Although upstream service will never be used, it's currently a mandatory
in Kong's data model and its hostname must be resolvable. So set
it to a fake value (ie, `http://127.0.0.1:20000`) on a [Service][service-object]
or an [API][api-object] if you are planning to use this plugin. Failing
to do so will result in 500 errors regarding a resolution failure.
