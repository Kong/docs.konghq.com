...
description: |
  ...
  <div class="alert alert-warning">
    ...
    underlying API, Route, or Service to be served over HTTPS. To avoid any
    confusion, we recommend that you configure your underlying API, Route, or
    Service to be only served through HTTPS.
    <br />
    <br />
...
params:
  ...
  config:
    ...
    - name: hide_credentials
      required: false
      default: "`false`"
      description: An optional boolean value telling the plugin to hide the credential to the upstream API, Route, or Service server. It will be removed by Kong before proxying the request
    ...
    - name: global_credentials
      ...
      default: "`false`"
      description: |
        An optional boolean value that allows to use the same OAuth credentials generated by the plugin with any other API, Route, or Service whose OAuth 2.0 plugin configuration also has `config.global_credentials=true`.
...
By default the OAuth 2.0 plugin listens on the following endpoints when consuming the API, Route, or Service on the [proxy port][proxy-port]:
...
The clients trying to authorize and request access tokens must use these endpoints. Remember that the endpoints above must be combined with the right URI path or headers that you normally use when consuming the root `/` endpoint of the API, Route, or Service through Kong.
...
When a client has been authenticated and authorized, the plugin will append some headers to the request before proxying it to the upstream API or Microservice, so that you can identify the consumer and the end-user in your code:
...
5. The backend must add the `provision_key` and `authenticated_userid` parameters to the `client_id`, `response_type` and `scope` parameters and it will make a `POST` request to Kong at your API address, on the `/oauth2/authorize` endpoint. If an `Authorization` header has been sent by the client, that must be added too. The equivalent of:
    ...
    The `provision_key` is the key the plugin has generated when it has been added to the API, Route, or Service, while `authenticated_userid` is the ID of the logged-in end user who has granted the permission.
...
9. Once the Access Token has been retrieved, the client application will make requests on behalf of the user to your final API, Route, or Service.
...
2. The backend of your web-application will authenticate the `username` and `password` sent by the client, and if successful will add the `provision_key`, `authenticated_userid` and `grant_type` parameters to the parameters originally sent by the client, and it will make a `POST` request to Kong at your API, Route, or Service address, on the `/oauth2/token` endpoint. If an `Authorization` header has been sent by the client, that must be added too. The equivalent of:
    ...
    The `provision_key` is the key the plugin has generated when it has been added to the API, Route, or Service, while `authenticated_userid` is the ID of the end user whose `username` and `password` belong to.
