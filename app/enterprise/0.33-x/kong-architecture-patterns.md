...
The next pattern we see, and the next step for security, is to setup [API servers behind a firewall](https://docs.konghq.com/0.13.x/network/) and only allow the Kong API Gateway be accessible to the public.
...
So long as the proxy nodes and admin nodes use the same database they will be part of the same cluster. The admin node itself can be located anywhere. There are a few different ways to achieve this. Read [securing the Kong Admin API](https://docs.konghq.com/0.13.x/secure-admin-api/) for details.
