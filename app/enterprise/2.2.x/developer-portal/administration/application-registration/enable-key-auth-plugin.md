...
| `Key in Header` | If enabled (default), the plugin reads the request header and tries to find the key in it. Default: true. **Note:** The field will be available in the GUI for the 2.3 release of Kong Enterprise. You can disable this feature now using a cURL command. See the [Key Auth](/hub/kong-inc/key-auth/) plugin for details. |
| `Key in Query` | If enabled (default), the plugin reads the query parameter in the request and tries to find the key in it. Default: true. **Note:** The field will be available in the GUI for the 2.3 release of Kong Enterprise. You can disable this feature now using a cURL command. See the [Key Auth](/hub/kong-inc/key-auth/) plugin for details.
