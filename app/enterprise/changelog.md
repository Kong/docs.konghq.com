...
### **Kong Enterprise**
The following sections list Enterprise-exclusive updates, features, and fixes
for the **2.2.0.0 beta** version.
...
(`proxy-cache-advanced`)
  ...
  parameters for confguring TLS connections.
...
- **Hybrid mode**:
  ...
  `default`, Kong Gateway did not present the configured cert when the correct
  SNI was sent.
...
- [OpenID Connect](/hub/kong-inc/openid-connect/) (`openid-connect`)
  ...
  - Fixed issue of sharing the cache key using the same credentials but with
    ...
    plugin instances.
  - Fixed issue which caused the `unexpected_redirect_uri` setting to be
  ignored.
