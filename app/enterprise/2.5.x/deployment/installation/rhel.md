...
2. Click a {{site.base_gateway}} version from the list to download it.
    ...
    For example: `kong-enterprise-edition-{{page.kong_versions[12].version}}.rhel8.noarch.rpm`
...
3. Copy the RPM file to your home directory on the RHEL system.
    ...
    ```bash
    $ scp kong-enterprise-edition-{{page.kong_versions[12].version}}.rhel8.noarch.rpm <rhel user>@<server>:~
    ```
