...
Enables or disables reporting the HTTP/Stream status codes per service/route by
the Prometheus plugin. Default `on`.
...
Enables or disables reporting the latency added by Kong, request time and
upstream latency by the Prometheus plugin. Default `on`.
...
Enables or disables the bandwidth consumed by service/route reporting by the
...
Enables or disables the upstream health status reporting by the Prometheus
plugin.
...
portal_smtp_admin_emails = Comma separated list of admin emails to receive
...
- `X-Kong-Request-Debug`: If the value is set to `*`, timing information will
  ...
  `upstream`: Collect timing information from the `upstream` phase. - `log`:
  Collect timing information from the `log` phase.
