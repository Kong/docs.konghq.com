...
Each migration has a unique name, and two functions which can execute [CQL](https://cassandra.apache.org/doc/latest/cql/index.html) statements: `up` and `down`. The up function is executed when Kong migrates your database (when starting or using the [kong migrations](/{{page.kong_version}}/cli#migrations) command).
