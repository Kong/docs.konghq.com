...
server will need to discover all Kong nodes by a service discovery mechanism,
and consume data from each node's configured `/metrics` endpoint.

### Grafana dashboard

Metrics exported by the plugin can be displayed by Grafana using a drop in
