...
configuration template](/gateway-oss/latest/configuration/#custom-nginx-configuration)
...
1. Kong Gateway users can protect the admin `/metrics` endpoint with an
   [RBAC user](/enterprise/latest/admin-api/rbac/reference/) that the
...
3. Finally, you could serve the content on a different port with a custom server
   ...
   template](/gateway-oss/latest/configuration/#custom-nginx-configuration) with Kong.
