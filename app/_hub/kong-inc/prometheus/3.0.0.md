...
- **Node info**: A guage metric set to 1 with the node's id and version; usable for discovery and version tracking.
...
* Metric names have been adjusted to add units to better standardize where possible
  * `http_status` -> `http_requests_total`
  * `latency` -> `kong_request_latency_ms`/`kong_upstream_latency_ms`/`kong_kong_latency_ms`
  * `kong_bandwidh` -> `kong_bandwidth_bytes`
  * `nginx_http_current_connections`/`nginx_stream_current_connections` -> `nginx_hconnections_total`
  * `http_consumer_status` removed.
...
* Latency has been split into 4 different metrics: `kong_latency_ms`, `upstream_latency_ms` and `request_latency_ms` (http) /`session_duration_ms` (stream). Buckets details below.
...
* The `node_id` label has been added to memory metrics.
