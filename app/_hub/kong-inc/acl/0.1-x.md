...
description: |
  Restrict access to a Service or a Route (or the deprecated API entity) by whitelisting or blacklisting consumers using arbitrary ACL group names. This plugin requires an [authentication plugin](/hub/#authentication) to have been already enabled on the Service or the Route (or API).
...
In order to use this plugin, you need to properly have configured your Service or Route (or API) with an [authentication plugin](/hub/#authentication) so that the plugin can identify who is the client [Consumer][consumer-object] making the request.
