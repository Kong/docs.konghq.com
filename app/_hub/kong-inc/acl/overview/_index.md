...
You can also enable the usage of `consumer-groups` by setting the config option `include_consumer_groups` to `true`.
This option will not only take the `ACL groups` but also the (Consumer Groups(link to page)) into consideration when evaluating
...
* [Examples](/hub/kong-inc/acl/how-to/examples/)
