...
will expire in less than the configured `renew_threshold_days` value. The renewal process
continues even when errors with individual certificates occur and retries the next
day. Note the renewal configuration is stored in the configured storage backend.
