...
params:
  ...
  config:
    ...
    - name: redis_username
      ...
      datatype: string
      description: |
        When using the `redis` policy, this property specifies the username to connect to the Redis server.
    - name: redis_password
      required: false
      datatype: string
      description: |
...
* Added parameters `redis_username`, `redis_ssl`, `redis_ssl_verify`, and `redis_server_name`.
