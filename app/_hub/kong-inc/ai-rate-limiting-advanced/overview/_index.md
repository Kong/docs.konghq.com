...
| `cost`   | Represents the cost calculated by multiplying the prompt and completions tokens generated by the LLM in response with the input and output cost indicated in the ai plugins: </br><code>cost = prompt_tokens * input_cost + completion_tokens * output_cost<code>.
