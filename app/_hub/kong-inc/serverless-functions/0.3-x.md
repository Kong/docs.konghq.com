...
- `pre-function`
  - Runs before other plugins run during the access phase.
- `post-function`
...
5. Apply the Lua code using the `pre-function` plugin with a cURL file upload:
...
7. Test the Lua code you just applied by making a valid request:
...
3. Test the Lua code you just applied by making a valid request:
...
in the access phase without the need to redeploy or restart Kong.
...
function. From version 0.3 onwards, a function can also be returned, which allows
for upvalues.
...
Because code is sent over in a string format, it is advisable to use either
