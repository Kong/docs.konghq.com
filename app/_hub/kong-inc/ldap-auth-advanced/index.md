...
params:
  ...
  config:
    ...
    - name: consumer_optional
      ...
      value_in_examples:
      description: |
        Whether consumer mapping is optional. If `consumer_optional=true`, then the plugin will not attempt to associate a consumer with the LDAP authenticated user. If `consumer_optional=false`, LDAP authenticated users can still access upstream resources. If you wish to prevent LDAP users who are not associated with consumers, then you must set `consumer_optional=false`, set the `anonymous` field to an existing consumer_id, and then use the Request Termination plugin to deny any requests from the anonymous consumer.
    - name: consumer_by
      required: false
