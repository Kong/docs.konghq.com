...
:-------------------------------------|:------------------------------|:----------------------
 ...
 [`vitals`](/gateway/latest/reference/configuration/#vitals) | n/a | Legacy vitals analytics reporting mechanism. Set to `off` for all {{site.base_gateway}} versions >= 3.0. Set to `on` for {{site.base_gateway}} 2.8.x to collect vitals data and send it to the control plane for Analytics dashboards and metrics.
