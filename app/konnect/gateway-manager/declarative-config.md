...
There are two different methods you can use to obtain the configuration file:

* Copy the autogenerated YAML configuration file from the {{site.konnect_short_name}} UI
* Create a configuration file with `decK dump`

### Copy the configuration file from the {{site.konnect_short_name}} UI

1. In {{site.konnect_short_name}}, navigate to **Gateway Manager** in the sidebar.

1. Click the control plane you want to copy the configuration file of and click **View configuration**. 

1. Ensure you are viewing the configuration file in YAML format, and then copy and paste the configuration file in a file named `kong.yaml`.
