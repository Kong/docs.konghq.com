...
2. **Claim Mappings**: Customize the mapping of required attributes to a different claim in the `id_token` {{site.konnect_short_name}} receives from the IdP. By default, {{site.konnect_short_name}} requires three attributes: Name, Email, and Groups. The values in these attributes are mapped as follows:
   ...
   {:.note}
> **Note:** If users are assigned a very large number of groups (over 150 in most cases), the IdP may send the groups claim in a non-standard manner which will cause authentication issues. To work around this limitation at the IdP, it is recommended to use group filtering functions provided by the IdP for this purpose. Quick reference guides: [Azure group filtering](https://learn.microsoft.com/en-us/azure/active-directory/hybrid/connect/how-to-connect-fed-group-claims#group-filtering) and [Okta group filtering](https://support.okta.com/help/s/article/How-to-send-certain-groups-that-the-user-is-assigned-to-in-one-Group-attribute-statement)https://support.okta.com/help/s/article/How-to-send-certain-groups-that-the-user-is-assigned-to-in-one-Group-attribute-statement. You may need to contact the Support team of your Identity Provider in order to see how to filter groups emitted for the application.
