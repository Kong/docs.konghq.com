...
> * Runtime Manager allows users to select the Gateway version that they want for their Quickstart scripts (except for cloud provider quickstart scripts for AWS, Azure and GCP). This allows customers to leverage official Konnect scripts to start their gateways while reducing the number of errors due to an invalid script for a certain gateway version.
