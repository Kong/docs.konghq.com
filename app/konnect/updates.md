...
: Users can now use Kong Gateway 3.0 runtimes with {{site.konnect_short_name}}. This allows {{site.konnect_short_name}} users to get access to the new capabilities and improvements added to Kong Gateway 3.0 core platform as well as the latest plugins that Kong Gateway 3.0 supports. Note: There are two Gateway dataplane features that are not yet supported at this time: Secret Management and Consumer Groups.
...
: {{site.konnect_short_name}} users can now generate personal access tokens (PATs). PATs are used as an alternative method of authentication for decK commands and Konnect API requests avoiding the need to use traditional username and passwords.

**AWS Marketplace Listing**
: {{site.konnect_short_name}} Enterprise can now be purchased through the [AWS Marketplace](https://aws.amazon.com/marketplace/pp/prodview-7zds3oxx3ntjy) or via private contract with your Amazon Web Services account team. This can greatly simplify the procuement process for existing AWS customers.

**DevPortal Custom Domain Progress Indicator**

: We've added an in interactive checklist when configuring a [custom domain](/konnect/dev-portal/customization/#custom-dev-portal-url) for your developer portal. Now you can get an indigation whether your DNS changes have propogated and when the SSL certificate has been generated by {{site.konnect_short_name}}.

**Download option to all reports**

: We have introduced an "Export" option to all reports inside the Overview page in Analytics. That will download an unpivoted CSV of the report to your local harddrive that can be used for offline analysis.
