...
Upstream Latency | The amount of time taken, measured in milliseconds, for {{site.base_gateway}} to wait for the first byte of the upstream service response. Users can select between different percentiles (p99, p95, and p50). For example, a 99th percentile request size of 100 bytes means that the payload size for every 1 in 100 requests was at least 100 bytes.
